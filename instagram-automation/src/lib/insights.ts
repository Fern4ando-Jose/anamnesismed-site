// Coleta de métricas (Insights) dos posts do @anamnesismed / @anamnesismed.es via
// Instagram Graph API.
//
// Por que existe: o "view" no grid do Instagram some com o contexto (idade do
// post, de onde veio o alcance, retenção). Para decidir cadência (Reel vs
// carrossel) precisamos de alcance + interações + saves/shares + tempo médio
// assistido (Reels), lado a lado e normalizado. Este módulo busca a mídia, puxa
// as insights por tipo e devolve as linhas enriquecidas + comparativo por formato.
//
// Lang-aware: cada conta (pt/es) tem seu token e account-id. Espelha o roteamento
// de getAccessToken/publishCarousel em /api/publish.
...
// Conta + token por idioma — espelha /api/publish. Token: config DB (refresh
// automático) com fallback p/ env. Account-id: env por idioma.
async function accountFor(lang: Lang): Promise<{ accountId?: string; token?: string }> {
  ...
  const envTok = lang === "es" ? process.env.META_ACCESS_TOKEN_ES : process.env.META_ACCESS_TOKEN;
  let token = envTok;
  try {
    const { sql } = await import("@vercel/postgres");
    const rows = await sql<{ value: string }>`SELECT value FROM config WHERE key = ${dbKey}`;
...
export async function getInsights(lang: Lang = "pt"): Promise<InsightsResult> {
  const generatedAt = new Date().toISOString();
  const { accountId, token } = await accountFor(lang);
  const tokenPresent = Boolean(accountId && token);

  if (!tokenPresent) {
    return {
      ok: false, lang, generatedAt, tokenPresent: false, items: [], byFormat: [],
      note: `Token/account-id da conta ${lang} ausentes (só existem em produção).`,
    };
  }
  ...
  try {
    ...
    return {
      ok: false, lang, generatedAt, tokenPresent: true, items: [], byFormat: [],
  ...
  // Comparativo por formato — a alavanca p/ decidir cadência (Reel vs carrossel).
  ...
  return { ok: true, lang, generatedAt, tokenPresent: true, items, byFormat };
}
