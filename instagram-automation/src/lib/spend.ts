...
//     (o /api/publish devolve HTTP 402 e o GitHub Actions falha com ::error::).
//
...
// sem precisar de deploy. Cadência atual: ig-posts = 3 PT + 3 ES/dia.
const DEFAULT_BUDGETS: Record<Automation, number> = {
  // 6 carrosséis/dia (3 BR + 3 ES) × (haiku + fal + QA sonnet) ≈ US$0,6/dia.
  "ig-posts": 1.0,
  "ig-reels": 0.25,
  manual: 0.5, // testes manuais (preview/dryrun) — cap evita o pico de dev.
};
...
export const EST_RUN_COST: Record<"publish" | "preview", number> = {
  publish: 0.15, // tavily + haiku + 3×(fal+QA)
  preview: 0.05, // haiku + 1×fal
};
...
// ok=false → a automação deve ser BLOQUEADA (route devolve HTTP 402).
